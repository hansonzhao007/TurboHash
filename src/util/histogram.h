...
struct HistogramStat {
    ...
    inline uint64_t sum_squares () const { return sum_squares_.load (std::memory_order_relaxed); }
...
class HistogramRocks {
public:
    HistogramRocks () {}
    virtual ~HistogramRocks (){};

    virtual void Clear () = 0;
    virtual bool Empty () const = 0;
    virtual void Add (uint64_t value) = 0;
    virtual void Merge (const HistogramRocks&) = 0;
...
class HistogramImpl : public HistogramRocks {
public:
    HistogramImpl () { Clear (); }
    ...
    virtual void Merge (const HistogramRocks& other) override;
...
void HistogramStat::Add (uint64_t value) {
    ...
    num_.store (num_.load (std::memory_order_relaxed) + 1, std::memory_order_relaxed);
    sum_.store (sum_.load (std::memory_order_relaxed) + value, std::memory_order_relaxed);
    sum_squares_.store (sum_squares_.load (std::memory_order_relaxed) + value * value,
                        std::memory_order_relaxed);
}
...
void HistogramStat::Merge (const HistogramStat& other) {
    ...
    while (other_min < old_min && !min_.compare_exchange_weak (old_min, other_min)) {
    }

    uint64_t old_max = max ();
    uint64_t other_max = other.max ();
    while (other_max > old_max && !max_.compare_exchange_weak (old_max, other_max)) {
    }
...
void HistogramImpl::Merge (const HistogramRocks& other) {
    if (strcmp (Name (), other.Name ()) == 0) {
        Merge (*static_cast_with_check<const HistogramImpl, const HistogramRocks> (&other));
    }
}
...
double HistogramImpl::Percentile (double p) const { return stats_.Percentile (p); }

double HistogramImpl::Average () const { return stats_.Average (); }

double HistogramImpl::StandardDeviation () const { return stats_.StandardDeviation (); }
