...
class Status {
 public:
  ...
  static Status Splitting(SubCode msg = kNone) {
    return Status(kSplitting, msg);
  }
  static Status Splitting(const Slice& msg,
                                   const Slice& msg2 = Slice()) {
    return Status(kSplitting, msg, msg2);
  }
  ...
  // Returns true iff current db is splitting
  bool IsSplitting() const {return code() == kSplitting;}
