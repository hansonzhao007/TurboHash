...
public:
    HashBench (int value_size)
        : max_count_ (FLAGS_num), key_trace_ (max_count_), value_ (value_size, 'v') {
        // arm_sig_int();
        signal (SIGALRM, &sigalrm_handler);  // set a signal handler
    }

    void PrintSpeed (std::string name, double loadfactor, size_t size, size_t inserted,
                     size_t duration_ns, bool read) {
        printf (
            "%33s - Load Factor: %.2f. %s   %10lu key, Size: %10lu. Speed: %6.2f Mops/s. Time: "
            "%6.2f s\n",
            name.c_str (), loadfactor, read ? "Read  " : "Insert", inserted, size,
            (double)inserted / duration_ns * 1000.0, duration_ns / 1000000000.0);
    }
    ...
    size_t TestRehash () {
        ...
        while (i < max_count_) {
            ...
            if ((i++ & 0xFFFFF) == 0) {
                fprintf (stderr, "insert%*s-%03d\r", int (i >> 20), " ", int (i >> 20));
                fflush (stderr);
            }
        }
        ...
        inserted_num = i;
        PrintSpeed (name, hashtable.LoadFactor (), hashtable.Size (), inserted_num,
                    time_end - time_start, false);
        ...
        auto read_fun = [&] {
            ...
            for (int t = 0; t < FLAGS_thread_read; t++) {
                workers[t] = std::thread ([&, t] {
                    ...
                    if (FLAGS_print_thread_read)
                        printf ("thread %2d trace offset: %10lu\n", t, start_offset);
                    ...
                    while (kRunning && key_iterator.Valid ()) {
                        ...
                        if ((i++ & 0xFFFFF) == 0) {
                            fprintf (stderr, "thread: %2d reading%*s-%03d->%s\r", t, int (i >> 20),
                                     " ", int (i >> 20), res->second ().c_str ());
                            fflush (stderr);
                        }
                    }
                    if (FLAGS_print_thread_read)
                        printf ("thread: %2d, search res: %s. iter info: %s. running: %s\n", t,
                                res != nullptr ? "true" : "false", key_iterator.Info ().c_str (),
                                kRunning ? "true" : "false");
                    counts[t] = i;
                });
            }
            ...
            PrintSpeed ("Get " + name, hashtable.LoadFactor (), hashtable.Size (), read_count,
                        time_end - time_start, true);
            if (FLAGS_print_thread_read)
        ...
        auto find_fun = [&] {
            ...
            for (int t = 0; t < FLAGS_thread_read; t++) {
                workers[t] = std::thread ([&, t] {
                    ...
                    while (kRunning && key_iterator.Valid ()) {
                        ...
                        if ((i++ & 0xFFFFF) == 0) {
                            fprintf (stderr, "thread: %2d finding%*s-%03d->%s\r", t, int (i >> 20),
                                     " ", int (i >> 20), record_ptr->second ().c_str ());
                            fflush (stderr);
                        }
                    }
                    if (FLAGS_print_thread_read)
                        printf ("thread: %2d, search res: %s. iter info: %s. running: %s\n", t,
                                record_ptr ? "true" : "false", key_iterator.Info ().c_str (),
                                kRunning ? "true" : "false");
                    counts[t] = i;
                });
            }
            ...
            auto time_end = util::NowNanos ();
            size_t read_count = std::accumulate (counts.begin (), counts.end (), 0);
            PrintSpeed ("Find " + name, hashtable.LoadFactor (), hashtable.Size (), read_count,
                        time_end - time_start, true);
            if (FLAGS_print_thread_read)
                for (int i = 0; i < FLAGS_thread_read; i++) {
                    printf ("thread %2d read: %10lu\n", i, counts[i]);
        ...
        auto probe_fun = [&] {
            ...
            for (int t = 0; t < FLAGS_thread_read; t++) {
                workers[t] = std::thread ([&, t] {
                    ...
                    while (kRunning && key_iterator.Valid ()) {
                        ...
                        if ((i++ & 0xFFFFF) == 0) {
                            fprintf (stderr, "thread: %2d probing%*s-%03d->%s\r", t, int (i >> 20),
                                     " ", int (i >> 20), val_ptr->second ().c_str ());
                            fflush (stderr);
                        }
                    }
                    if (FLAGS_print_thread_read)
                        printf ("thread: %2d, search res: %s. iter info: %s. running: %s\n", t,
                                val_ptr != nullptr ? "true" : "false",
                                key_iterator.Info ().c_str (), kRunning ? "true" : "false");
                    counts[t] = i;
                });
            }
            std::for_each (workers.begin (), workers.end (), [] (std::thread& t) { t.join (); });
            auto time_end = util::NowNanos ();
            size_t read_count = std::accumulate (counts.begin (), counts.end (), 0);
            PrintSpeed ("Probe " + name, hashtable.LoadFactor (), hashtable.Size (), read_count,
                        time_end - time_start, true);
            if (FLAGS_print_thread_read)
                for (int i = 0; i < FLAGS_thread_read; i++) {
                    printf ("thread %2d read: %10lu\n", i, counts[i]);
        ...
        // (double)hashtable.Size() / (time_end - time_start), (double)(time_end - time_start) /
        // 1000000.0 );
    ...
    size_t TurboHashSpeedTest () {
        ...
        size_t max_range = max_count_ * FLAGS_loadfactor;
        {
            ...
            for (int t = 0; t < FLAGS_thread_write; t++) {
                workers[t] = std::thread ([&, t] {
                    ...
                    while (res && i < max_range) {
                        ...
                        if ((i++ & 0xFFFFF) == 0) {
                            fprintf (stderr, "thread: %2d inserting%*s-%03d->\r", t, int (i >> 20),
                                     " ", int (i >> 20));
                            fflush (stderr);
                        }
                    }
            ...
            printf ("Total put: %lu\n", write_count);
            PrintSpeed (name, hashtable.LoadFactor (), hashtable.Size (), write_count,
                        time_end - time_start, false);
        }

        {
            auto read_fun = [&] {
                ...
                kRunning = true;
                if (FLAGS_readtime != 0)
                    alarm (FLAGS_readtime);  // set an alarm for 6 seconds from now
                auto time_start = util::NowNanos ();
                for (int t = 0; t < FLAGS_thread_read; t++) {
                    workers[t] = std::thread ([&, t] {
                        size_t i = 0;
                        ...
                        while (kRunning && key_iterator.Valid ()) {
                            res = hashtable.Find (key_iterator.Next ());
                            if ((i++ & 0xFFFFF) == 0) {
                                fprintf (stderr, "thread: %2d reading%*s-%03d->%s\r", t,
                                         int (i >> 20), " ", int (i >> 20),
                                         res->second ().c_str ());
                                fflush (stderr);
                            }
                ...
                auto time_end = util::NowNanos ();
                size_t read_count = std::accumulate (counts.begin (), counts.end (), 0);
                PrintSpeed (name, hashtable.LoadFactor (), hashtable.Size (), read_count,
                            time_end - time_start, true);
                if (FLAGS_print_thread_read)
                    for (int i = 0; i < FLAGS_thread_read; i++) {
                        printf ("thread %2d read: %10lu\n", i, counts[i]);
    ...
    void HashSpeedTest (const std::string& name, size_t inserted_num) {
        ...
        while (key_iterator.Valid ()) {
            ...
            if ((i++ & 0xFFFFF) == 0) {
                fprintf (stderr, "inserting%*s-%03d->\r", int (i >> 20), " ", int (i >> 20));
                fflush (stderr);
            }
        }
        auto time_end = util::NowNanos ();
        PrintSpeed (name.c_str (), map.load_factor (), map.size (), inserted_num,
                    time_end - time_start, false);

        {
            ...
            for (int t = 0; t < FLAGS_thread_read; t++) {
                workers[t] = std::thread ([&, t] {
                    ...
                    while (kRunning && key_iterator.Valid () && iter != map.end ()) {
                        ...
                        if ((i++ & 0xFFFFF) == 0) {
                            fprintf (stderr, "thread: %2d reading%*s-%03d->%s\r", t, int (i >> 20),
                                     " ", int (i >> 20), value.c_str ());
                            fflush (stderr);
                        }
                    }
                    if (FLAGS_print_thread_read)
                        printf ("thread: %2d, search res: %s. iter info: %s. running: %s\n", t,
                                iter != map.end () ? "true" : "false",
                                key_iterator.Info ().c_str (), kRunning ? "true" : "false");
                    counts[t] = i;
                });
            }
            ...
            size_t read_count = std::accumulate (counts.begin (), counts.end (), 0);
            PrintSpeed (name.c_str (), map.load_factor (), map.size (), read_count,
                        time_end - time_start, true);
            if (FLAGS_print_thread_read)
                for (int i = 0; i < FLAGS_thread_read; i++) {
                    printf ("thread %2d read: %10lu\n", i, counts[i]);
...

        {
            ...
            time_end = util::NowNanos ();
            size_t read_count = std::accumulate (counts.begin (), counts.end (), 0);
            PrintSpeed ("Probe " + name, map.load_factor (), map.size (), read_count,
                        time_end - time_start, true);
            if (FLAGS_print_thread_read)
                for (int i = 0; i < FLAGS_thread_read; i++) {
                    printf ("thread %2d read: %10lu\n", i, counts[i]);
    ...
    void CuckooSpeedTest (const std::string& name, size_t inserted_num) {
        ...
        auto time_end = util::NowNanos ();
        PrintSpeed (name.c_str (), map.load_factor (), map.size (), i, time_end - time_start,
                    false);
        ...
        for (int t = 0; t < FLAGS_thread_read; t++) {
            workers[t] = std::thread ([&, t] {
                ...
                do {
                    ...
                    if ((i++ & 0xFFFFF) == 0) {
                        fprintf (stderr, "thread: %2d reading%*s-%03d->\r", t, int (i >> 20), " ",
                                 int (i >> 20));
                        fflush (stderr);
                    }
                ...
                if (FLAGS_print_thread_read)
                    printf ("thread: %2d, search res: %s. iter info: %s, running: %s\n", t,
                            is_find ? "true" : "false", key_iterator.Info ().c_str (),
                            kRunning ? "true" : "false");
                counts[t] = i;
            });
        }
...
int main (int argc, char* argv[]) {
    ...
    // printf("Inserted: %lu\n", inserted_num);
    // hash_bench.HashSpeedTest<robin_hood::unordered_map<size_t, std::string>, std::string
    // >("robin_hood::unordered_map", FLAGS_num);
    // hash_bench.HashSpeedTest<absl::flat_hash_map<size_t, std::string>, std::string
    // >("absl::flat_hash_map", FLAGS_num); hash_bench.HashSpeedTest<std::unordered_map<size_t,
    // std::string>, std::string >("std::unordered_map", FLAGS_num);
