  ...
  void Report() {
    ...
    for (int i = 0; i < dimm_num; i++) {
      ...
      TotalMediaReads_.at(i) =
          (end->pmm_dimms_.at(i).stat_[TotalMediaReads - 1].l_u64b -
    ...
    printf(
        ...
        "\n");
    printf(
        "DIMM  | Read from IMC | Write from IMC |   Read DIMM |   Write DIMM | "
        "    RA |     WA "
        "|\n");
    for (int i = 0; i < dimm_num; i++) {
      printf("  %-2d  | %13.2f | %14.2f | %11.2f | %12.2f | %6.2f | %6.2f |\n",
             i, TotalReadRequests_.at(i), TotalWriteRequests_.at(i),
             TotalMediaReads_.at(i), TotalMediaWrites_.at(i),
             (TotalMediaReads_.at(i) / TotalReadRequests_.at(i)),
