...
public:
	...
	static void
	AppendWithSpace(std::string *str, const std::string &msg)
	{
		if (msg.empty())
			return;
		if (!str->empty()) {
	...
	void
	AddMessage(const std::string &msg)
	{
		AppendWithSpace(&message_, msg);
	}

	inline void
	FinishedBatchOp(size_t batch)
	{
		double now = NowNanos();
		last_op_finish_ = now;
		done_ += batch;
	...
	void
	Report(const Slice &name, bool print_hist = false)
	{
		...
		printf("%-12s : %11.3f micros/op %lf Mops/s;%s%s\n",
		       ...
		       throughput / 1024 / 1024, (extra.empty() ? "" : " "),
		       extra.c_str());
		if (print_hist) {
...
public:
	...
	Done(int64_t increment)
	{
		...
		if (max_seconds_) {
			// Recheck every appx 1000 ops (exact iff increment is
			// factor of 1000)
			auto granularity = 1000;
			if ((ops_ / granularity) !=
			    ((ops_ - increment) / granularity)) {
				uint64_t now = NowMicros();
				return ((now - start_at_) / 1000000) >=
					max_seconds_;
			} else {
				return false;
			}
...
public:
	      ...
	      key_trace_(nullptr)
	{
		...
		pop_ = nvobj::pool<root>::create(FLAGS_filepath.c_str(), LAYOUT,
						 PMEMOBJ_MIN_POOL * 40960,
						 S_IWUSR | S_IRUSR);
		auto proot = pop_.root();
		{
			...
			proot->cons =
				nvobj::make_persistent<persistent_map_type>(
					(uint64_t)LEVEL_HASH_POWER, 1);
#elif defined TYPE_CLHT
			proot->cons =
				nvobj::make_persistent<persistent_map_type>(
					(uint64_t)CLHT_N_BUCKETS);
#endif
	...
	inline Ret
	Find(const string_t &key, size_t tid)
	{
#ifdef TYPE_CCEH
	...
	inline Ret
	Delete(const string_t &key, size_t tid)
	{
#ifdef TYPE_CLEVEL
	...
	inline Ret
	Update(const string_t &key, const string_t &val, size_t tid)
	{
#ifdef TYPE_CCEH
		return map_->insert((uint8_t *)key.c_str(),
				    (uint8_t *)val.c_str(), key.size(),
				    val.size(), tid);
#elif defined TYPE_CLEVEL
	...
	inline Ret
	Insert(const string_t &key, const string_t &val, size_t tid)
	{
#ifdef TYPE_CCEH
		return map_->insert((uint8_t *)key.c_str(),
				    (uint8_t *)val.c_str(), key.size(),
	...
	DoRead(ThreadState *thread)
	{
		...
		auto key_iterator =
			key_trace_->trace_at(start_offset, trace_size_);
		size_t not_find = 0;
		...
		while (!duration.Done(batch) && key_iterator.Valid()) {
			...
			for (; j < batch && key_iterator.Valid(); j++) {
				...
				auto ret = Find({key.data(), key.size()},
						thread->tid);
				if (!ret.found) {
					not_find++;
				}
		...
		char buf[100];
		snprintf(buf, sizeof(buf), "(num: %lu, not find: %lu)", reads_,
			 not_find);
		thread->stats.AddMessage(buf);
	}

	void
	DoReadNon(ThreadState *thread)
	{
		uint64_t batch = FLAGS_batch;
		if (key_trace_ == nullptr) {
		...
		size_t start_offset = random() % trace_size_;
		auto key_iterator =
			key_trace_->nontrace_at(start_offset, trace_size_);
		size_t not_find = 0;
		Duration duration(FLAGS_readtime, reads_);
		thread->stats.Start();
		while (!duration.Done(batch) && key_iterator.Valid()) {
	...
	void
	DoReadNonLat(ThreadState *thread)
	{
		if (key_trace_ == nullptr) {
			perror("DoReadLat lack key_trace_ initialization.");
			return;
	...
	DoWrite(ThreadState *thread)
	{
		...
		auto key_iterator = key_trace_->iterate_between(
			start_offset, start_offset + interval);

		thread->stats.Start();
		std::string val(value_size_, 'v');
		while (key_iterator.Valid()) {
			for (uint64_t j = 0; j < batch && key_iterator.Valid();
			     j++) {
				std::string &key = key_iterator.Next();

				auto res = Insert({key.data(), key.size()},
						  {key.data(), key.size()},
						  thread->tid);
			}
	...
	void
	DoLoadFactor(ThreadState *thread)
	{
		uint64_t batch = FLAGS_batch;
		if (key_trace_ == nullptr) {
	...
	void
	DoOverWrite(ThreadState *thread)
	{
		uint64_t batch = FLAGS_batch;
		if (key_trace_ == nullptr) {
		...
		while (key_iterator.Valid()) {
			...
			for (; j < batch && key_iterator.Valid(); j++) {
				...
				auto res = Update({key.data(), key.size()},
						  {key.data(), key.size()},
						  thread->tid);
				if (res.found) {
					// success update
	...
	DoDelete(ThreadState *thread)
	{
		...
		while (key_iterator.Valid()) {
			...
			for (; j < batch && key_iterator.Valid(); j++) {
				std::string &key = key_iterator.Next();
				auto res = Delete({key.data(), key.size()},
						  thread->tid);
				if (res.found) {
					// success deleted
					deleted++;
	...
	YCSBA(ThreadState *thread)
	{
		...
		char buf[100];
		snprintf(buf, sizeof(buf), "(insert: %lu, read: %lu)", insert,
			 find);
		thread->stats.AddMessage(buf);
		return;
	}
	...
	YCSBD(ThreadState *thread)
	{
		...
		// Read the latest 20%
		auto key_iterator = key_trace_->iterate_between(
			start_offset + 0.8 * interval, start_offset + interval);
		printf("thread %2d, between %lu - %lu\n", thread->tid,
		       (uint64_t)(start_offset + 0.8 * interval),
	...
	YCSBF(ThreadState *thread)
	{
		...
		char buf[100];
		snprintf(buf, sizeof(buf), "(read_modify: %lu, read: %lu)",
			 insert, find);
		thread->stats.AddMessage(buf);
		return;
	}
...
private:
	...
	void
	RunBenchmark(int thread_num, const std::string &name,
	...
	PrintEnvironment()
	{
		...
		if (cpuinfo != nullptr) {
			...
			while (fgets(line, sizeof(line), cpuinfo) != nullptr) {
				...
				std::string key = TrimSpace(
					std::string(line, sep - 1 - line));
				std::string val =
					TrimSpace(std::string(sep + 1));
				if (key == "model name") {
					++num_cpus;
					cpu_type = val;
