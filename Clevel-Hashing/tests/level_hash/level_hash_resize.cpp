...
main(int argc, char *argv[])
{
...

	{
		...
		proot->cons = nvobj::make_persistent<persistent_map_type>((uint64_t)N_LEVELS, 1);
	...
	fout = fopen("level_hash_exp2_load_factor.csv", "w");
	...
	while (getline(&pbuf, &len, ycsb) != -1) {
		if (strncmp(buf, "INSERT", 6) == 0) {
			...
			if (!ret.found) {
				loaded++;
				// if (loaded % 1000 == 0)
				// 	std::cout << "[SUCCESS] inserted " << loaded << std::endl;
