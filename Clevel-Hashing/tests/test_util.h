...
public:
	...
	{
		int res = system(
			("ipmctl show -performance > " + filename).c_str());
		...
		std::regex stat_bit_convert_reg(
		...
		while (ipmctl_stat >> str_line) {
			for (size_t i = 0; i < reg_set.size(); i++) {
				if (std::regex_search(str_line, matched_data,
						      reg_set.at(i))) {
					index = i;
					break;
				}
			...
			if (index == DimmID) {
				...
				if (std::regex_search(str128b, matched_num,
						      stat_bit_convert_reg)) {
					pmm_dimms_.back()
						.stat_[index - 1]
						.h_u64b = std::stoull(
						matched_num[1], nullptr, 16);
					pmm_dimms_.back()
						.stat_[index - 1]
						.l_u64b = std::stoull(
						matched_num[2], nullptr, 16);
				} else {
...
public:
	...
	float *outer_imc_read_addr = nullptr, *outer_imc_write_addr = nullptr,
	      *outer_media_read_addr = nullptr,
	      *outer_media_write_addr = nullptr;
	...
	Report()
	{
		...
		float media_read_size_MB = 0, imc_read_size_MB = 0,
		      imc_write_size_MB = 0, media_write_size_MB = 0;
		...
		std::vector<float> TotalMediaReads_(dimm_num),
			TotalMediaWrites_(dimm_num),
			TotalReadRequests_(dimm_num),
			TotalWriteRequests_(dimm_num);
		for (int i = 0; i < dimm_num; i++) {
			for (int j = 0; j < 8; j++)
				assert(start->pmm_dimms_.at(i)
					       .stat_[j]
					       .h_u64b ==
				       end->pmm_dimms_.at(i).stat_[j].h_u64b);
			TotalMediaReads_.at(i) =
				(end->pmm_dimms_.at(i)
					 .stat_[TotalMediaReads - 1]
					 .l_u64b -
				 start->pmm_dimms_.at(i)
					 .stat_[TotalMediaReads - 1]
					 .l_u64b) /
				16384.0;
			TotalMediaWrites_.at(i) =
				(end->pmm_dimms_.at(i)
					 .stat_[TotalMediaWrites - 1]
					 .l_u64b -
				 start->pmm_dimms_.at(i)
					 .stat_[TotalMediaWrites - 1]
					 .l_u64b) /
			...
			TotalMediaReads_.at(i) = TotalMediaReads_.at(i) -
				TotalMediaWrites_.at(i);
		...
		if (outer_imc_read_addr)
			*outer_imc_read_addr = imc_read_size_MB;
		if (outer_imc_write_addr)
			*outer_imc_write_addr = imc_write_size_MB;
		if (outer_media_read_addr)
			*outer_media_read_addr = media_read_size_MB;
		if (outer_media_write_addr)
			*outer_media_write_addr = media_write_size_MB;

		printf("--------------------------------------------------------------------------------------"
		       "\n");
		printf("DIMM  | Read from IMC | Write from IMC |   Read DIMM |   Write DIMM |     RA |     WA "
		       "|\n");
		for (int i = 0; i < dimm_num; i++) {
			printf("  %-2d  | %13.2f | %14.2f | %11.2f | %12.2f | %6.2f | %6.2f |\n",
			       i, TotalReadRequests_.at(i),
			       TotalWriteRequests_.at(i),
			       TotalMediaReads_.at(i), TotalMediaWrites_.at(i),
			       (TotalMediaReads_.at(i) /
				TotalReadRequests_.at(i)),
			       (TotalMediaWrites_.at(i) /
				TotalWriteRequests_.at(i)));
		...
		printf(" SUM:\n"
		       ...
		       "   Time: %7.1f s\n",
		       media_read_size_MB / seconds, media_read_size_MB,
		       imc_read_size_MB / seconds, imc_read_size_MB,
		       media_write_size_MB / seconds, media_write_size_MB,
...
public:
	...
	friend std::ostream &
	operator<<(std::ostream &os, const Slice &str)
	{
		os << str.ToString();
		return os;
	}
...
static uint64_t
u64Rand(const uint64_t &min, const uint64_t &max)
{
...
public:
	RandomKeyTrace(size_t count)
	{
		...
		tbb::parallel_for(
			...
			[&](const tbb::blocked_range<uint64_t> &range) {
				for (uint64_t i = range.begin();
				     i != range.end(); i++) {
					// uint64_t num =
					// 	u64Rand(1LU, kRandNumMax);
					keys_[i] = i;
				}
			});
		auto duration =
	...
	Randomize(void)
	{
		...
		auto starttime = std::chrono::system_clock::now();
		tbb::parallel_for(
			tbb::blocked_range<uint64_t>(0, keys_.size()),
			[&](const tbb::blocked_range<uint64_t> &range) {
	...
	class RangeIterator {
	public:
		RangeIterator(std::vector<size_t> *pkey_vec, size_t start,
			      size_t end)
	...
	public:
		...
		Info()
		{
			char buffer[128];
			sprintf(buffer,
				"valid: %s, cur i: %lu, end_i: %lu, range: %lu",
				Valid() ? "true" : "false", cur_index_,
				end_index_, range_);
			return buffer;
		}
	...
	Iterator
	trace_at(size_t start_index, size_t range)
	{
	...
	RangeIterator
	iterate_between(size_t start, size_t end)
	{
...
public:
	RandomKeyTraceString(size_t count) : count_(count)
	{
		...
		auto starttime = std::chrono::system_clock::now();
		tbb::parallel_for(
			tbb::blocked_range<uint64_t>(0, N_STRS),
			[&](const tbb::blocked_range<uint64_t> &range) {
				for (uint64_t i = range.begin();
				     i != range.end(); i++) {
					keys_[i].reserve(S_LEN);
					std::generate_n(
						std::back_inserter(keys_[i]),
						S_LEN, [&]() {
							return alphabet[u64Rand(
								0, 61)];
						});
				}
			});
	...
	class RangeIterator {
	public:
		RangeIterator(std::vector<std::string> *pkey_vec, size_t start,
			      size_t end)
		    : pkey_vec_(pkey_vec), end_index_(end), cur_index_(start)
		{
		}
	...
	class Iterator {
	public:
		Iterator(std::vector<std::string> *pkey_vec, size_t start_index,
			 size_t range)
		    : pkey_vec_(pkey_vec),
		      range_(range),
		      end_index_(start_index % range_),
	...
	Iterator
	nontrace_at(size_t start_index, size_t range)
	{
