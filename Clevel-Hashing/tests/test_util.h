...
public:
	...
	Report()
	{
		...
		for (int i = 0; i < dimm_num; i++) {
			printf("DIMM%d | Read from IMC | Write from IMC |  Read DIMM  |  Write "
			       "DIMM  |   "
			       "RA   |   WA   |\n",
			       i);
		...
		printf("*SUM* | DIMM-R: %7.1f MB/s. User-R: %7.1f MB/s   | DIMM-W: %7.1f "
		       "MB/s, "
		       "User-W: %7.1f MB/s. Time: %6.2fs\n",
		       media_read_size_MB / seconds, imc_read_size_MB / seconds,
		       media_write_size_MB / seconds,
